	...
	h.prototype = {
		...
		_createNavigation: function() {
			...
			for (var k = 0; k < j.slides.length; k++) {
				d('<li data-num="' + k + '"><button class="pogoSlider-nav-btn"></button></li>')
					.appendTo(l)
					.on('click', function() {
		...
		_slideElementCleanup: function(m) {
			var j = function(n, o) {
				return (o.match(/pogoSlider-(?:(?:transition)|(?:animation))(?:-[a-zA-Z0-9]+)?(?:--[a-z]+)?/gi) || [])
					.join(' ');
			};
			var l = function(n, o) {
		...
		slideTransitions: {
			...
			slideRevealLeft: function(k, m) {
				...
				var l = j.slides[m];
				j.slides[k].$element
					.precss({ '*z-index': j.settings.baseZindex + 1, 'animation-duration': l.duration + 'ms' })
					.addClass('pogoSlider-animation-leftOut');
		...
		compatSlideTransitions: {
			...
			slideOverLeft: function(j, l) {
				var k = this.slides[l];
				k.$element
					.css({ opacity: 1, 'z-index': this.settings.baseZindex, left: '100%' })
					.animate({ left: 0 }, k.duration);
			},
			slideOverRight: function(j, l) {
				var k = this.slides[l];
				k.$element
					.css({ opacity: 1, 'z-index': this.settings.baseZindex, right: '100%' })
			...
			expandReveal: function(j, l) {
				...
				this.slides[j].$element
					.css('z-index', this.settings.baseZindex + 1)
					.animate({ width: '120%', height: '120%', left: '-10%', top: '-10%', opacity: 0 }, k.duration);
				k.$element.css({ opacity: 1, 'z-index': this.settings.baseZindex });
			},
			shrinkReveal: function(j, l) {
				...
				this.slides[j].$element
					.css('z-index', this.settings.baseZindex + 1)
					.animate({ width: '50%', height: '50%', left: '25%', top: '25%', opacity: 0 }, k.duration);
				k.$element.css({ opacity: 1, 'z-index': this.settings.baseZindex });
			},
			verticalSplitReveal: function(l, n) {
				var k = this;
